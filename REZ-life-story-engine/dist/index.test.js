...
describe('LifeStoryService', () => {
    ...
    describe('addChapter', () => {
        it('should create a new story and add chapter if none exists', async () => {
            ...
            const chapter = await service.addChapter(userId, 'First Chapter', ['Event 1', 'Event 2'], ['joy', 'excitement'], ['growth']);
            expect(chapter).toBeDefined();
            expect(chapter.chapters.length).toBeGreaterThan(0);
            expect(chapter.chapters[0].title).toBe('First Chapter');
        });
        it('should add chapter to existing story', async () => {
            const userId = 'user-add-existing';
            await service.generateStory(userId);
            const updatedStory = await service.addChapter(userId, 'New Chapter', ['Event A'], ['anticipation'], ['exploration']);
    ...
    describe('getChapter', () => {
        ...
        it('should return the correct chapter', async () => {
            ...
            const updatedStory = await service.addChapter(userId, 'Target Chapter', ['Target Event'], ['focus'], ['target']);
            const story = await service.getStory(userId);
            const targetChapterId = updatedStory.chapters.find(c => c.title === 'Target Chapter')?.id;
            if (targetChapterId) {
    ...
    describe('updateChapter', () => {
        ...
        it('should update chapter with provided fields', async () => {
            ...
            const story = await service.addChapter(userId, 'Original Title', ['Original Event'], ['neutral'], ['original']);
            const chapterToUpdate = story.chapters[0];
            const updatedChapter = await service.updateChapter(story.id, chapterToUpdate.id, {
                title: 'Updated Title',
    ...
    describe('deleteChapter', () => {
        ...
        it('should return true and remove chapter when found', async () => {
            ...
            const story = await service.addChapter(userId, 'To Be Deleted', ['Event'], ['sadness'], ['deletion']);
    ...
    describe('getThemes', () => {
        ...
        it('should return all themes from user story', async () => {
            ...
            await service.addChapter(userId, 'Chapter 1', ['Event 1'], ['emotion1'], ['theme1', 'theme2']);
            await service.addChapter(userId, 'Chapter 2', ['Event 2'], ['emotion2'], ['theme2', 'theme3']);
    ...
    describe('summarizeLife', () => {
        ...
        it('should return summary with themes and events', async () => {
            ...
            await service.addChapter(userId, 'Chapter One', ['Notable Event 1', 'Notable Event 2'], ['joy', 'pride'], ['major-theme', 'minor-theme']);
        ...
        it('should respect maxChapters limit', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await service.addChapter(userId, `Chapter ${i + 1}`, [`Event ${i + 1}`], ['emotion'], [`theme-${i}`]);
            }
