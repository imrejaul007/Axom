...
import { getHours, getDay, getMonth, differenceInDays, startOfWeek } from 'date-fns';
...
export const LifeEventModel = mongoose.model('LifeEvent', lifeEventSchema);
...
// ROUTINE DETECTION
// ============================================
export async function detectRoutines(userId, events) {
    if (events.length < 5)
        return [];
    ...
    for (const [activityType, activityEvents] of Object.entries(activityGroups)) {
        ...
        const routine = {
            ...
            outcomes: [],
            detectedAt: new Date(),
            lastActive: new Date(Math.max(...activityEvents.map((e) => new Date(e.timestamp).getTime()))),
        };
...
// ============================================
export function analyzeTimePattern(events) {
    const hourScores = {};
    const dayScores = {};
    const seasonScores = {};
    const durations = [];
...
// LIFESTYLE PROFILE
// ============================================
export function analyzeLifestyleProfile(events) {
    if (events.length === 0) {
        return {
    ...
    if (earlyMorning > lateNight * 2)
        chronotype = 'early_bird';
    else if (lateNight > earlyMorning * 2)
    ...
    // Work-life balance
    const workEvents = events.filter(e => e.activity?.toLowerCase().includes('work') ||
        e.type?.toLowerCase().includes('work')).length;
...
// CONSUMPTION PATTERN
// ============================================
export function analyzeConsumptionPattern(events) {
    const cuisinePrefs = {};
    const diningTimes = {
        early_morning: 0,
        morning: 0,
    ...
    for (const event of events) {
        ...
        if (event.location?.name) {
            ...
            if (name.includes('home'))
                diningLocations.home = (diningLocations.home || 0) + 1;
            else if (name.includes('restaurant') || name.includes('cafe'))
                diningLocations.restaurant = (diningLocations.restaurant || 0) + 1;
            else if (name.includes('delivery'))
                diningLocations.delivery = (diningLocations.delivery || 0) + 1;
            else if (name.includes('office') || name.includes('work'))
                diningLocations.work = (diningLocations.work || 0) + 1;
        }
    }
    ...
    return {
        ...
        diningTimes,
        diningLocation: diningLocations,
        budgetTier,
...
// ============================================
export function analyzeRhythms(events) {
    // Weekly rhythm
...
function analyzeRhythm(events, units, type) {
    const scores = new Array(units).fill(0);
    const counts = new Array(units).fill(0);
    for (const event of events) {
        const timestamp = new Date(event.timestamp);
        let unitIndex;
        if (type === 'weekly') {
...
// ============================================
export function detectLifeStage(events) {
    if (events.length === 0) {
        return {
...
// ROUTINE PREDICTION
// ============================================
export function predictNext24hRoutines(userId, timePattern, routines) {
    const predictions = [];
    const now = new Date();
...
// ============================================
export function detectLifeEvents(signals) {
    const detected = [];
    ...
    for (const [type, typeSignals] of Object.entries(typeGroups)) {
        if (typeSignals.length >= 5) {
            ...
            const timestamps = typeSignals.map((s) => new Date(s.timestamp).getTime());
...
function mapActivityToLifeEvent(activity) {
    const lower = activity.toLowerCase();
    if (lower.includes('work') && lower.includes('new'))
        return 'new_job';
    if (lower.includes('travel') || lower.includes('vacation'))
        return 'travel';
    if (lower.includes('home') || lower.includes('apartment'))
        return 'relocation';
    if (lower.includes('date') || lower.includes('relationship'))
...
// ============================================
export async function generateLifePatternContext(userId, events, includePredictions = true) {
    // Get stored routines
    ...
    const todayRoutines = routines.filter(r => r.pattern.daysOfWeek.includes(DAY_MAPPINGS[today]));
...
// HELPER FUNCTIONS
// ============================================
function findRoutineGaps(timePattern, routines) {
    const gaps = [];
...
function generateSuggestions(timePattern, lifestyle, rhythms) {
    const suggestions = [];
    // From chronotype
...
function predictUpcomingChanges(lifeStage, rhythms) {
    const changes = [];
...
let lifestyle = null;
//# sourceMappingURL=lifePatternService.js.map