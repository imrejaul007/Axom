...
import { PatternStatus, PatternType, } from "../types.js";
/**
...
export class PatternService {
     ...
     */
    async recordEvent(userId, type, location, context, metadata) {
        const event = {
            id: uuidv4(),
            userId,
            type,
     ...
     */
    async updatePattern(patternId) {
        const pattern = this.patterns.get(patternId);
        if (!pattern) {
            throw new Error(`Pattern not found: ${patternId}`);
        }
        const userEvents = this.getUserEvents(pattern.userId);
        const recentEvents = this.filterRecentEvents(userEvents, pattern.type, pattern.lastObserved);
        // Recalculate confidence based on recent data
        const newDataPoints = recentEvents.length;
        const newConfidence = this.calculateConfidence(newDataPoints, pattern.occurrences);
        ...
        const daysSinceLastObserved = Math.floor((Date.now() - pattern.lastObserved.getTime()) / MS_PER_DAY);
     ...
     */
    async makePrediction(userId, patternId, predictedFor, prediction) {
        const pattern = this.patterns.get(patternId);
        if (!pattern) {
            throw new Error(`Pattern not found: ${patternId}`);
        }
    ...
    async getAccuracy(userId) {
        ...
        const completedPredictions = userPredictions.filter((p) => p.accuracy !== undefined);
     ...
     */
    async getBehaviorSummary(userId, days = 30) {
        const cutoffDate = new Date(Date.now() - days * MS_PER_DAY);
        const userEvents = this.getUserEvents(userId).filter((e) => e.timestamp >= cutoffDate);
     ...
     */
    analyzeEventSequence(events, eventType) {
        // Sort by timestamp
        ...
        for (let i = 1; i < sortedEvents.length; i++) {
            intervals.push(sortedEvents[i].timestamp.getTime() -
        ...
        const confidence = this.calculateConfidence(sortedEvents.length, sortedEvents.length);
        // Determine status
     ...
     */
    determinePatternType(avgInterval) {
    ...
    calculateAverageTime(events) {
        const totalHour = events.reduce((sum, e) => sum + e.timestamp.getHours(), 0);
     ...
     */
    calculateConfidence(dataPoints, occurrences) {
     ...
     */
    filterRecentEvents(events, type, since) {
