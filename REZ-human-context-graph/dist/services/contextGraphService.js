...
const humanContextSchema = new Schema({
...
export const HumanContextModel = mongoose.model('HumanContext', humanContextSchema);
...
export const SignalModel = mongoose.model('LayerSignal', signalSchema);
// ============================================
// SIGNAL COLLECTION
// ============================================
export async function collectLayerSignal(userId, layer, signal, value, source, confidence = 0.5, metadata) {
    const layerSignal = {
        layer,
        source,
        signal,
...
async function updateLayerContext(userId, signal) {
...
// CONTEXT AGGREGATION
// ============================================
export async function aggregateHumanContext(userId, requestedLayers) {
    ...
    if (emotionalResult.status === 'fulfilled' && emotionalResult.value) {
        humanContext.spiritual = emotionalResult.value;
    ...
    if (lifePatternResult.status === 'fulfilled' && lifePatternResult.value) {
        humanContext.daily = lifePatternResult.value;
    ...
    if (commerceResult.status === 'fulfilled' && commerceResult.value) {
        humanContext.commerce = commerceResult.value;
    ...
    if (careerResult.status === 'fulfilled' && careerResult.value) {
        humanContext.career = careerResult.value;
    ...
    if (mobilityResult.status === 'fulfilled' && mobilityResult.value) {
        humanContext.mobility = mobilityResult.value;
    ...
    if (wellnessResult.status === 'fulfilled' && wellnessResult.value) {
        humanContext.health = wellnessResult.value;
...
// ============================================
async function fetchEmotionalContext(userId) {
    try {
        const response = await axios.get(`${SERVICE_URLS.emotional}/api/context`, { data: { userId }, timeout: 5000 });
        ...
        if (signals.length > 0) {
            return {
                layer: 'spiritual',
                emotionalState: signals[0].signal || 'neutral',
                stressLevel: 50,
...
async function fetchLifePatternContext(userId) {
    try {
        const response = await axios.post(`${SERVICE_URLS.lifePattern}/api/context`, { userId }, { timeout: 5000 });
        const data = response.data;
        return {
            layer: 'daily',
...
async function fetchCommerceContext(userId) {
    try {
        // Get psychology profile
        const response = await axios.get(`${process.env.BEHAVIORAL_PSYCHOLOGY_URL || 'http://localhost:4110'}/api/psychology/${userId}/scores`, { timeout: 5000 });
        const data = response.data;
        return {
            layer: 'commerce',
            spendingLevel: 50,
...
async function fetchMobilityContext(userId) {
    try {
        const signals = await SignalModel.find({
            userId,
            layer: 'mobility',
        ...
        const travelSignals = signals.filter((s) => s.signal.includes('travel'));
        const commuteSignals = signals.filter((s) => s.signal.includes('commute'));
        return {
            layer: 'mobility',
...
async function fetchWellnessContext(userId) {
    try {
        const response = await axios.get(`${SERVICE_URLS.emotional}/api/wellness/${userId}`, { timeout: 5000 });
        const data = response.data;
        return {
            layer: 'health',
...
// ============================================
function generateCrossLayerInsights(context) {
    const insights = [];
...
function assessLifeStage(context) {
...
function identifyRiskFactors(context) {
    const risks = [];
    // Health risks
    if (context.health) {
...
function identifyOpportunities(context) {
    const opportunities = [];
    // Health opportunity
...
// ============================================
function generateLayerSummary(layer, signals) {
    if (signals.length === 0)
...
// ============================================
export async function generateCosmicContext(humanContext) {
...
// COSMIC HELPERS
// ============================================
function generateSymbolicInterpretation(domain, score) {
    const interpretations = {
        health: {
...
function generateSuggestedActions(context) {
    const actions = [];
...
function generateTimingAdvice(context) {
...
function generateAvoidedActions(context) {
    const avoided = [];
...
function generateAbstractInsights(context) {
    const insights = [];
...
//# sourceMappingURL=contextGraphService.js.map