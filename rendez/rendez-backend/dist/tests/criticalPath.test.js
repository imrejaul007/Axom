...
function makeToken(profileId, rezUserId = 'rez_mock_user') {
    return jsonwebtoken_1.default.sign({ sub: profileId, rezUserId }, JWT_SECRET, { expiresIn: '1h' });
}
// ── Test State ─────────────────────────────────────────────────────────────────
let aliceToken;
let bobToken;
...
describe('Rendez Critical Path', () => {
    ...
    describe('1. Auth — REZ token verification', () => {
        ...
        it('rejects invalid/missing tokens', async () => {
            const res = await (0, supertest_1.default)(index_1.default).post('/api/v1/auth/verify');
    ...
    describe('3. Discovery and mutual like → Match', () => {
        ...
        it('matches list includes the new match with unreadCount', async () => {
            const res = await (0, supertest_1.default)(index_1.default)
                ...
                .set('Authorization', `Bearer ${aliceToken}`);
            expect(res.status).toBe(200);
            const match = res.body.find((m) => m.id === matchId);
    ...
    describe('5. Gift flow — send, accept, unlock', () => {
        ...
        it('Bob can see the gift in his inbox', async () => {
            const res = await (0, supertest_1.default)(index_1.default)
                ...
                .set('Authorization', `Bearer ${bobToken}`);
            expect(res.status).toBe(200);
            const found = res.body.find((g) => g.id === giftId);
    ...
    describe('8. Admin — stats and moderation', () => {
        it('GET /admin/stats returns all KPIs', async () => {
            const res = await (0, supertest_1.default)(index_1.default).get('/admin/stats');
            expect(res.status).toBe(200);
        ...
        it('GET /admin/stats/timeseries returns 7-day data', async () => {
            const res = await (0, supertest_1.default)(index_1.default).get('/admin/stats/timeseries?days=7');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
        ...
        it('GET /admin/reports returns pending reports', async () => {
            const res = await (0, supertest_1.default)(index_1.default).get('/admin/reports?status=PENDING');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
        it('GET /admin/gifts returns gift list', async () => {
            const res = await (0, supertest_1.default)(index_1.default).get('/admin/gifts');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
    });
...
//# sourceMappingURL=criticalPath.test.js.map