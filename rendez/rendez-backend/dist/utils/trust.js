...
exports.computeTrustSignals = computeTrustSignals;
function computeTrustSignals(p) {
    let trustLevel;
    if (!p.isVerified)
        trustLevel = 'UNVERIFIED';
    else if (p.meetupCount >= 6)
        trustLevel = 'GOLD';
    else if (p.meetupCount >= 3)
        trustLevel = 'SILVER';
    else if (p.meetupCount >= 1)
        trustLevel = 'BRONZE';
    else
        trustLevel = 'VERIFIED';
    ...
    if (pct >= 95)
        ...
        responseLabel = 'SLUGGISH';
    const hoursSince = (Date.now() - new Date(p.lastActiveAt).getTime()) / (1000 * 60 * 60);
    let activeLabel;
    if (hoursSince < 24)
        activeLabel = 'ACTIVE_TODAY';
    else if (hoursSince < 168)
        activeLabel = 'ACTIVE_THIS_WEEK';
    else if (hoursSince < 720)
        activeLabel = 'ACTIVE_THIS_MONTH';
    else
