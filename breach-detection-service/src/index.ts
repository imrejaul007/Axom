...
async function checkHIBPBreaches(email: string): Promise<HIBPBreach[]> {
  if (!HIBP_API_KEY) {
    console.warn('HIBP API key not configured, using mock data');
...
async function checkHIBPPastes(email: string): Promise<HIBPPaste[]> {
  ...
  try {
    ...
    console.error('HIBP Paste API error:', error.message);
    return [];
  }
}
...
async function getAllBreaches(): Promise<HIBPBreach[]> {
  ...
  try {
    ...
    console.error('HIBP All Breaches API error:', error);
    return getMockBreachDefinitions();
  }
}
...
app.post('/check/phone', async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error('Phone breach check error:', error);
...
║  Status:      RUNNING                    ║
║  Port:        ${PORT}                          ║
║  HIBP API:   ${HIBP_API_KEY ? 'CONFIGURED' : 'NOT CONFIGURED'}          ║
