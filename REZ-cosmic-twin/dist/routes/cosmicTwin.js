 ...
 */
router.post("/create", validateRequest(z.object({
 ...
 */
router.get("/:id", validateRequest(z.object({ id: z.string().uuid() }), "params"), async (req, res, next) => {
    try {
 ...
 */
router.get("/user/:userId", validateRequest(z.object({ userId: z.string().min(1) }), "params"), async (req, res, next) => {
    try {
        const twin = await cosmicTwinService.getByUserId(req.params.userId);
        if (!twin) {
            throw new AppError(404, `No twin found for user "${req.params.userId}"`);
        }
        res.json({ data: twin });
    }
    catch (err) {
 ...
 */
router.post("/:id/sync", validateRequest(z.object({
    id: z.string().uuid(),
    data: z.unknown(),
}), "body"), async (req, res, next) => {
    try {
        const syncRecord = await cosmicTwinService.sync(req.params.id, req.body.data);
 ...
 */
router.post("/:id/capability", validateRequest(z.object({
    id: z.string().uuid(),
    capability: z.nativeEnum(TwinCapability),
}), "body"), async (req, res, next) => {
    try {
        const twin = await cosmicTwinService.addCapability(req.params.id, req.body.capability);
        res.json({ data: twin });
    }
    catch (err) {
        next(err);
 ...
 */
router.get("/:id/sync", validateRequest(z.object({ id: z.string().uuid() }), "params"), async (req, res, next) => {
    try {
 ...
 */
router.get("/status/:status", validateRequest(z.object({ status: z.nativeEnum(TwinStatus) }), "params"), async (req, res, next) => {
    try {
        const twins = await cosmicTwinService.getByStatus(req.params.status);
        res.json({ data: twins });
    }
    catch (err) {
        next(err);
 ...
 */
router.get("/:id/learning", validateRequest(z.object({ id: z.string().uuid() }), "params"), async (req, res, next) => {
    try {
        const progress = await cosmicTwinService.getLearningProgress(req.params.id);
 ...
 */
router.delete("/:id", validateRequest(z.object({ id: z.string().uuid() }), "params"), async (req, res, next) => {
    try {
...
//# sourceMappingURL=cosmicTwin.js.map