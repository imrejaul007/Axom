...
import { communityRoutes, postRoutes } from './routes/index.js';
...
app.use('/communities', postRoutes);

// Health check
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'buzzlocal-community-service', environment: nodeEnv });
});
...
const start = async () => {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`BuzzLocal Community Service running on port ${PORT}`);
