...
// ============================================
export function interpretMoodToCosmicState(mood, energy) {
    const moodMap = {
...
function getRelationshipGuidance(socialEnergy) {
    if (socialEnergy > 80)
        return 'Strong connection energy - ideal for meaningful conversations';
    if (socialEnergy > 60)
        return 'Favorable for social engagement and networking';
    if (socialEnergy > 40)
        return 'Moderate social energy - quality over quantity';
    return 'Solitude may feel more restorative today';
}
function getFinancialGuidance(energyLevel) {
    if (energyLevel === 'high')
        return 'Abundance mindset flows naturally - wise decisions favored';
    if (energyLevel === 'medium')
        return 'Balanced approach serves well - steady progress';
    return 'Conservation may feel wiser - avoid impulsive commitments';
}
function getGrowthInsight(energyLevel) {
    if (energyLevel === 'high')
        return 'Momentum is building - this is a favorable time for new beginnings';
    if (energyLevel === 'medium')
        return 'Steady progress continues - consistency compounds';
    return 'This phase may be for integration rather than expansion';
}
// ============================================
// AI COUNCIL - AGENT INTERPRETATIONS
// ============================================
export function generateCouncilResponse(cosmicState, input, activeAgents = ['mystic', 'healer', 'strategist', 'oracle']) {
    const insights = [];
...
function generateAgentInsight(agentType, cosmicState, input) {
    switch (agentType) {
        case 'mystic':
...
function generateMysticInsight(state, input) {
...
function generateHealerInsight(state, input) {
...
function generateStrategistInsight(state, input) {
...
function generateOracleInsight(state, input) {
    const patterns = [
...
function generateConnectorInsight(state, input) {
...
function generateWealthGuideInsight(state, input) {
...
function generateExplorerInsight(state, input) {
    if (state.energyLevel === 'high') {
        return {
            agent: 'explorer',
...
function generateConsensus(insights, state) {
...
// ============================================
export function generateDailyReading(state, userId) {
...
// DOMAIN GUIDANCE
// ============================================
export function getDomainGuidance(domain, state, input) {
    const domainMap = {
        career: {
            domain,
...
// ============================================
export function processMoodCheckIn(checkIn) {
    const moodMap = {
        radiant: {
            ...
            insight: { agent: 'mystic', category: 'opportunity', title: 'Radiant Energy', interpretation: 'This light is meant to be shared', symbolic: 'Your radiance touches all you encounter', practical: 'Channel this energy into meaningful pursuits', confidence: 0.9 },
        },
        bright: {
            ...
            insight: { agent: 'mystic', category: 'guidance', title: 'Bright Outlook', interpretation: 'Optimism creates its own momentum', symbolic: 'Light reveals what darkness hides', practical: 'Act on positive impulses with wisdom', confidence: 0.85 },
        },
        balanced: {
            ...
            insight: { agent: 'healer', category: 'guidance', title: 'Centered Balance', interpretation: 'Equilibrium is its own wisdom', symbolic: 'Balance is the foundation of all growth', practical: 'Maintain this equilibrium with gentle awareness', confidence: 0.9 },
        },
        clouded: {
            ...
            insight: { agent: 'healer', category: 'guidance', title: 'Clouds Before Rain', interpretation: 'Even clouds carry gifts of nourishment', symbolic: 'The rain that follows will bring new growth', practical: 'Be gentle with yourself - clarity returns', confidence: 0.8 },
        },
        stormy: {
            ...
            insight: { agent: 'healer', category: 'warning', title: 'Storm Passing', interpretation: 'The storm cleanses as it passes through', symbolic: 'After every storm comes the calm', practical: 'Breathe through intensity - this too shall pass', confidence: 0.85 },
        },
        peaceful: {
            ...
            insight: { agent: 'healer', category: 'guidance', title: 'Inner Peace', interpretation: 'This peace is a treasure - savor it', symbolic: 'Peace is the destination of all journeys', practical: 'Share this calm energy gently with others', confidence: 0.9 },
        },
        restless: {
            ...
            insight: { agent: 'explorer', category: 'opportunity', title: 'Restless Seeking', interpretation: 'This restlessness points toward something important', symbolic: 'The search itself is part of the finding', practical: 'Channel restlessness into curiosity rather than anxiety', confidence: 0.75 },
        },
        tired: {
            ...
            insight: { agent: 'healer', category: 'guidance', title: 'Honoring Fatigue', interpretation: 'Tiredness asks for listening, not pushing', symbolic: 'The deepest roots grow in darkness', practical: 'Prioritize rest above productivity today', confidence: 0.9 },
        },
    };
    ...
    const adjustedState = {
        ...mapped.state,
        energyLevel: checkIn.energy >= 4 ? 'high' : checkIn.energy >= 2 ? 'medium' : 'low',
    };
    return {
        recorded: true,
...
// ============================================
export async function generateCosmicContext(userId) {
    ...
    try {
        const response = await axios.post(`${SERVICE_URLS.emotional}/api/context`, { userId, includeCosmic: true }, { timeout: 3000 });
...
function generateSuggestedActions(state, council) {
    const actions = [];
...
function generateAvoidedActions(state) {
    const avoided = [];
    if (state.energyLevel === 'high') {
...
//# sourceMappingURL=cosmicService.js.map